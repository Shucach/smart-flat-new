...
(function (e) {
    'function' === typeof define && define.amd
        ...
        : 'object' === typeof exports
          ? (module.exports = function (k, j) {
                k || (k = window);
                if (!j || !j.fn.dataTable)
                    j = require('datatables.net')(k, j).$;
                return e(j, k, k.document);
            })
    ...
    var g = e.fn.dataTable,
        l = function (a, b) {
            ...
            this.c = e.extend(!0, {}, g.defaults.keyTable, l.defaults, b);
            this.s = { dt: new g.Api(a), enable: !0 };
            this.dom = {};
    ...
    e.extend(l.prototype, {
        ...
        _constructor: function () {
            ...
            e(j.body).on('keydown.keyTable', function (b) {
                a._key(b);
            });
            if (this.c.blurable)
                e(j.body).on('click.keyTable', function (c) {
                    e(c.target).parents('.dataTables_filter').length &&
                        a._blur();
                    e(c.target).parents().filter(b.table().container())
                        .length ||
                        e(c.target).parents('div.DTE').length ||
                        a._blur();
                });
            if (this.c.editor)
                b.on('key.kt', function (b, d, e, h, i) {
                    a._editor(e, i);
                });
            if (b.settings()[0].oFeatures.bStateSave)
                b.on('stateSaveParams.keyTable', function (b, d, e) {
                    e.keyTable = a.s.lastFocus ? a.s.lastFocus.index() : null;
                });
            b.on('destroy.keyTable', function () {
                b.off('.keyTable');
                e(b.table().body()).off('click.keyTable', 'th, td');
                e(j.body).off('keydown.keyTable').off('click.keyTable');
            });
            (d = b.state.loaded()) && d.keyTable
                ? b.cell(d.keyTable).focus()
        ...
        _focus: function (a, b) {
            ...
            if (this.s.enable) {
                ...
                if (-1 !== f.length && (a < f.start || a >= f.start + f.length))
                    ...
                    f.serverSide && (a -= f.start);
                    f = c.cell(':eq(' + a + ')', b, { search: 'applied' });
                    if (m) {
                        if (m.node() === f.node()) return;
                        this._blur();
                    }
                    m = e(f.node());
                    m.addClass(this.c.className);
                    this._scroll(e(k), e(j.body), m, 'offset');
                    h = c.table().body().parentNode;
                    h !== c.table().header().parentNode &&
                        ((h = e(h.parentNode)),
                        this._scroll(h, h, m, 'position'));
                    this.s.lastFocus = f;
                    this._emitEvent('key-focus', [this.s.dt, f]);
                    c.state.save();
                }
            }
        },
            ...
            if (
                ...
                if (b) {
                    ...
                    if (
                        !(
                            this.c.keys &&
                            -1 === e.inArray(a.keyCode, this.c.keys)
                        )
                    )
                        switch (a.keyCode) {
                            case 9:
        ...
        _scroll: function (a, b, d, c) {
            ...
            c.left < h && b.scrollLeft(c.left);
            c.top + f > e + i && b.scrollTop(c.top + f - i);
            c.left + d > h + a && b.scrollLeft(c.left + d - a);
        },
        _shift: function (a, b, d) {
            ...
            if (h) {
                ...
                c = c.columns(i).indexes().indexOf(h.index().column);
                f = i[c];
                'right' === b
                    ? c >= i.length - 1
                        ? (g++, (f = i[0]))
                    ...
                    : 'left' === b
                      ...
                      : 'up' === b
                        ? g--
                        : 'down' === b && g++;
                0 <= g && g < j && -1 !== e.inArray(f, i)
                    ? (a.preventDefault(), this._focus(g, f))
                    : !d || !this.c.blurable
                      ? a.preventDefault()
                      : this._blur();
            }
        ...
        _tabInput: function () {
            ...
            if (-1 != d)
                e('<div><input type="text" tabindex="' + d + '"/></div>')
                    .css({
                        position: 'absolute',
                        height: 1,
                        width: 0,
                    ...
                    .children()
                    .on('focus', function () {
                        a._focus(b.cell(':eq(0)', { page: 'current' }));
                    });
        },
    });
    l.defaults = {
    ...
    e(j).on('preInit.dt.dtk', function (a, b) {
        if ('dt' === a.namespace) {
            var d = b.oInit.keys,
                c = g.defaults.keys;
