...
(function ($) {
    ...
    function getMockForRequest(handler, requestSettings) {
        ...
        if ($.isFunction(handler.url.test)) {
            ...
            if (
                (handler.url !== requestSettings.url && star === -1) ||
                !new RegExp(
                    handler.url
                        .replace(/[-[\]{}()+?.,\\^$|#\s]/g, '\\$&')
                        .replace(/\*/g, '.+'),
        ...
        if (handler.data) {
            if (
                ...
                !isMockDataEqual(handler.data, requestSettings.data)
            ) {
        ...
        var process = (function (that) {
            return function () {
                return function () {
                    ...
                    if (
                        typeof mockHandler.status == 'number' ||
                    ...
                    if ($.isFunction(onReady)) {
                        ...
                        onReady.call(
                            this,
                            mockHandler.isTimeout ? 'timeout' : undefined,
                        );
    ...
    function processJsonpMock(requestSettings, mockHandler, origSettings) {
        ...
        if (
            ...
            if (requestSettings.type.toUpperCase() === 'GET' && remote) {
                var newMockReturn = processJsonpRequest(
                    requestSettings,
                    mockHandler,
                    origSettings,
                );
    ...
    function processJsonpRequest(requestSettings, mockHandler, origSettings) {
        ...
        var callbackContext =
                (origSettings && origSettings.context) || requestSettings,
            newMock = null;
    ...
    $.mockjaxSettings = {
        ...
        log: function (mockHandler, requestSettings) {
            ...
            if (window.console && console.log) {
                var message =
                    'MOCK ' +
                    requestSettings.type.toUpperCase() +
                    ': ' +
                    requestSettings.url;
