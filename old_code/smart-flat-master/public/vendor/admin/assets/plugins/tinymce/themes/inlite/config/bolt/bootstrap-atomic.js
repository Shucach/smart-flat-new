        ...
        function () {
            ...
            var forall = function (a, f) {
                ...
                for (var i = 0; i < a.length; ++i)
                    if (fn(a[i]) !== true) return false;
                return true;
            };

            var map = function (a, f) {
                var r = [];
                for (var i = 0; i < a.length; ++i) r.push(f(a[i], i));
                return r;
            };
            ...
            var indexof = function (a, x) {
                for (var i = 0; i < a.length; ++i) if (a[i] === x) return i;
                return -1;
            };
    ...
    kernel.fp.object = def(
        ...
        function () {
            var map = function (o, f) {
                var r = {};
                for (var i in o) if (o.hasOwnProperty(i)) r[i] = f(i, o[i]);
                return r;
            };
            ...
            var amap = function (data, f, oncomplete) {
                ...
                ar.each(data, function (datum, i) {
                    f(datum, function (result) {
                        ++count;
                        results[i] = result;
                        if (count === total) oncomplete(results);
                    });
                });
            };
                ...
                var get = function (id, define, require) {
                    ...
                    var load = function (onsuccess, onfailure) {
                        ...
                        if (instance !== undefined) {
                            ...
                            onfailure(
                                'Modulator error: could not resolve global [' +
                                    name +
                                    ']',
                            );
                        }
        ...
        function (array) {
            ...
            var analyse = function (roots, modules) {
                ...
                var decycle = function (name) {
                    if (array.contains(path, name)) cycle = collect(path, name);
                    else descend(name);
                };
        ...
        function (ar, fn, map, piggybacker, stratifier) {
            var create = function (
                ...
                var validate = function (onsuccess, results) {
                    ...
                    if (failed.length > 0)
                        onerror(
                            'Fetcher error: modules were not defined: ' +
                                failed.join(', '),
                        );
                    else onsuccess();
                };
    ...
    kernel.module.loader = def(
        ...
        function (analyser) {
            var load = function (
                ...
                else if (result.load.length === 0) onsuccess();
                else fetcher.fetch(result.load, oncontinue);
            };
                ...
                var demand = function (id) {
                    ...
                    if (result === undefined)
                        throw (
                            "module '" +
                            id +
                            "' returned undefined from definition function"
                        );
                    modules[id] = result;
                    return result;
                };
            ...
            var create = function (builtins, configuration) {
                ...
                var find = function (id) {
                    for (var i = 0; i < sources.length; ++i)
                        if (sources[i].can(id)) return { found: sources[i] };
        ...
        function (ar, fn) {
            var create = function (sources) {
                ...
                var regulate = function (
                    ...
                    var unresolved = ar.filter(
                        required,
                        fn.not(sources.isResolved),
                    );
                ...
                var resolve = function (
                    ...
                    for (var i = 0; i < ids.length; ++i) {
                        ...
                        if (source.notfound) {
                            onerror(
                                'Could not find source for module [' + id + ']',
                            );
                            return;
                        }
                        var spec = source.found.get(
                            id,
                            define,
                            require,
                            demand,
    ...
    loader.tag.script = def(
        ...
        function () {
            ...
            var onload = function (el, callback) {
                ...
                else el.addEventListener('load', callback, false);
            };
        ...
        function () {
            var read = function (url, success, error) {
                ...
                fs.exists(url, function (exists) {
                    ...
                    else error('File does not exist [' + url + ']');
                });
            };
    ...
    loader.transporter.xhr = def(
        ...
        function () {
            ...
            var handler = function (req, url, success, error) {
                return function () {
                    if (req.readyState === 4) done(req, url, success, error);
                };
            };
            ...
            var request = function (url, success, error) {
                ...
                if (req) getUrl(req, url, success, error);
                else
                    error(
                        'Transport error: browser does not support XMLHttpRequest.',
                    );
            };
