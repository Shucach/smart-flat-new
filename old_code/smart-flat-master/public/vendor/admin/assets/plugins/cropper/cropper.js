...
(function (factory) {
    ...
    $.extend(prototype, {
        ...
        start: function () {
            this.image = getImageData(
                this.isImg ? this.$element[0] : this.$clone[0],
            );
            this.ready = true;
            this.build();
        },
    ...
    $.extend(prototype, {
        ...
        renderCanvas: function (changed) {
            ...
            canvas.width = min(
                max(canvas.width, canvas.minWidth),
                canvas.maxWidth,
            );
            canvas.height = min(
        ...
        limitCropBox: function (size, position) {
            ...
            if (position) {
                if (strict) {
                    ...
                    cropBox.maxLeft =
                        min(containerWidth, canvas.left + canvas.width) -
                        cropBox.width;
                    cropBox.maxTop =
                        min(containerHeight, canvas.top + canvas.height) -
                        cropBox.height;
                } else {
                    cropBox.minLeft = 0;
                    cropBox.minTop = 0;
    ...
    $.extend(prototype, {
        bind: function () {
            ...
            $document
                .on(
                    EVENT_MOUSE_MOVE,
                    (this._cropMove = proxy(this.cropMove, this)),
                )
                .on(
                    EVENT_MOUSE_UP,
                    (this._cropEnd = proxy(this.cropEnd, this)),
                );

            if (options.responsive) {
                $window.on(
                    EVENT_RESIZE,
                    (this._resize = proxy(this.resize, this)),
                );
            }
        },
    ...
    $.extend(prototype, {
        ...
        cropMove: function (event) {
            ...
            if (action) {
                ...
                this.change(
                    e.shiftKey,
                    action === ACTION_ZOOM ? originalEvent : null,
                );
            }
        },
