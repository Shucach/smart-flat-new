!(function () {
    ...
    d3.selectAll = function (nodes) {
        var group = d3_array(
            typeof nodes === 'string'
                ? d3_selectAll(nodes, d3_document)
                : nodes,
        );
        group.parentNode = d3_documentElement;
        return d3_selection([group]);
    ...
    d3.geom.voronoi = function (points) {
        ...
        function voronoi(data) {
            var polygons = new Array(data.length),
                ...
                x1 = clipExtent[1][0],
                y1 = clipExtent[1][1];
            d3_geom_voronoi(sites(data), clipExtent).cells.forEach(
                function (cell, i) {
    ...
    d3_svg_lineInterpolators.forEach(function (key, value) {
        value.key = key;
        value.closed = key.endsWith('-closed');
    });
    ...
    d3.svg.brush = function () {
        ...
        function redraw(g) {
            g.selectAll('.resize').attr('transform', function (d) {
                return (
                    'translate(' +
                    xExtent[+d.endsWith('e')] +
                    ',' +
                    yExtent[+d.startsWith('s')] +
                    ')'
                );
            });
        }
        ...
        function brushstart() {
            ...
            if (dragging) {
                ...
                var ex = +resizing.endsWith('w'),
                    ey = +resizing.startsWith('n');
                offset = [
